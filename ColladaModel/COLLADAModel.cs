...
namespace ColladaSlimDX.ColladaModel
{
    ...
    public class Model
    {
        ...
        public class InstanceCamera
        {
            ...
            {
                Matrix viewMatrix = parentBone.GetAbsoluteTransformMatrix(time);
                
                return viewMatrix;
            }
            
            public InstanceCamera(Camera camera, Bone parentBone)
            {
                this.camera = camera;
        ...
        public class Bone
        {
            ...
            public Matrix GetTransformMatrix(float time) {
            ...
            public Matrix GetAbsoluteTransformMatrix(float time) {
                if (Parent == null)
                    return GetTransformMatrix(time);
                
                return GetTransformMatrix(time) * Parent.GetAbsoluteTransformMatrix(time);
            }
        ...
        public class SkinnedInstanceMesh : InstanceMesh
        {
            ...
            public List<Matrix> GetSkinningMatrices(float time) {
                ...
                for (int i = 0; i < invBindMatrixList.Count && i < MaxSkinningMatrixCount; i++)
                    boneMatrixList.Add(bindShapeMatrix * invBindMatrixList[i] * bones[i].GetAbsoluteTransformMatrix(time));
                            ...
                            case "JOINT":
                                ...
                                switch (source.arrayType)
                                {
                                    case "IDREF_array":
                                        ...
                                        {
                                            if (model.BonesTable.TryGetValue(keys[i], out bone))
                                                bones.Add(bone);
                                            else
                                                throw new ColladaException("Bone with id '" + keys[i] + "' not found in bone list.");
                                        }
                                        break;
                                    case "Name_array":
                                        ...
                                        for (int i = 0; i < keys.Count; i++)
                                        {
                                            ...
                                            if (model.BonesTable.TryGetValue(node.id, out bone))
                                                ...
                                                throw new ColladaException("Bone with id '" + node.id + "' not found in bone list.");
                                        }
                                        break;
                                    default:
                                        COLLADAUtil.Log(COLLADALogType.Error, "Unkown array type '" + source.arrayType + "' in skinning controller.");
                                        break;
                                }
                                break;
        ...
        // A collection of Camera objects used by this model.
        ...
        // A collection of InstanceCameras
        public List<InstanceCamera> InstanceCameras { get { return instanceCameras; } }
        ...
        public Matrix ConversionMatrixInverse
        {
            get
            {
                Matrix inverse = ConversionMatrix;
                inverse.Invert();
                return inverse;
            }
        }
